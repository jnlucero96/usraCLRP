...
def generate_plot(file_list):
    ...
    full_data = pydose.DoseFile(file_name_1,load_uncertainty=True)
    phantom_seed_data = pydose.DoseFile(file_name_2,load_uncertainty=True)

    fig_x, ax_x = plt.subplots(20, 20, figsize=(
    ...
    for j in xrange(20):
        for k in xrange(20):
            ...
            ax_x[j, k].plot(np.linspace(x_min, x_max, Nx),
    ...
    fig_y, ax_y = plt.subplots(20,20,figsize=(30,30),sharex='all',sharey='all')
    ...
    for i in xrange(20):
        for k in xrange(20):
            ...
            ax_y[i,k].plot(np.linspace(y_min, y_max, Ny), full_data.dose[i,:,k], lw=3.0, label='Full Geometry')
            ax_y[i,k].plot(np.linspace(y_min, y_max, Ny),
    ...
    fig_z, ax_z = plt.subplots(20, 20, figsize=(
        30, 30), sharex='all', sharey='all')
    ...
    for i in xrange(20):
        for j in xrange(20):
            ...
            ax_z[i, j].plot(np.linspace(z_min, z_max, Nz),
...
def generate_plot2(file_list):
    ...
    full_prostate_data = pydose.DoseFile(file_name_1,load_uncertainty=True)
    phantom_seed_data = pydose.DoseFile(file_name_2,load_uncertainty=True)
    water_pad_data = pydose.DoseFile(file_name_3, load_uncertainty=True)
    ...
    for i in xrange(full_prostate_data.dose[:, 0, 0].size):
        for k in xrange(full_prostate_data.dose[0, 0, :].size):
            ...
            ax_y[i,k].plot(np.linspace(y_min, y_max, Ny), full_prostate_data.dose[i,:,k], lw=3.0, label='Full Prostate Geometry')
            ax_y[i,k].plot(np.linspace(y_min, y_max, Ny),
                    phantom_seed_data.dose[i,:,k], lw=3.0, ls='--', label='Scored Phantom only')
            ax_y[i,k].plot(np.linspace(y_min, y_max, Ny),
...
def generate_dose_histogram(file_list):
    ...
    fig,ax = plt.subplots(4,1,figsize=(10,10),sharex='col',sharey='all')
    ...
    fig2,ax2 = plt.subplots(4,1,figsize=(10,10),sharex='col',sharey='all')
    ...
    fig2.text(
        0.35,0.97,'Cumulative Dose Histograms',fontsize=20,va='center'
    )
...
if __name__ == "__main__":
    program_name, arguments = sys.argv[0], sys.argv[1:]
