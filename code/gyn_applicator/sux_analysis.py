...
def generate_tdvh_mlwa():
    ...
    file_list = [
        file_90_shield_2point0mm,
        file_180_shield_2point0mm,
    ...
    for index in xrange(3):
        ...
        data_flat = main_data.dose.flatten() # converts to Gy; norm to individual max dwell time
...
def generate_tdvh_tg43():
    ...
    file_tg43_0point5mm = target_dir + \
        '/tg43_0shield_0.5mm_sim.phantom_wo_box.egsinp'
    file_tg43_1point0mm = target_dir + \
        '/tg43_0shield_1mm_sim.phantom_wo_box.egsinp'
    file_tg43_2point0mm = target_dir + \
        '/tg43_0shield_2mm_sim.phantom_wo_box.egsinp'

    file_tg43_applicator_0point5mm = target_dir + \
        '/tg43_applicator_0.5mm_sim.phantom_wo_applicator_wo_box.egsinp'
    file_tg43_applicator_1point0mm = target_dir + \
        '/tg43_applicator_1mm_sim.phantom_wo_applicator_wo_box.egsinp'
    file_tg43_applicator_2point0mm = target_dir + \
        '/tg43_applicator_2mm_sim.phantom_wo_applicator_wo_box.egsinp'
    ...
    for index1 in xrange(2):
        for index2 in xrange(3):
            ...
            # converts to Gy; norm to treatment time
            # data_base_flat = main_base_data.dose.flatten() * 2.2861e14 
...
def dose_position_plots():
    ...
    file_list = [
        ...
        target_dir +
        '/mlwa_90shield_{0}_sim_sux.phantom.3ddose',
        target_dir +
        '/mlwa_180shield_{0}_sim_sux.phantom.3ddose',
        target_dir +
        '/mlwa_270shield_{0}_sim_sux.phantom.3ddose',
    ]
    ...
    for fig_index in xrange(1):
            ...
            for index1 in xrange(3):  # iterate through axes
                ...
                if index1 == 0:
                    ax[index1].plot(
                        ...
                        lw=3.0, label=label_list[index2]
                    )
                elif index1 == 1:
                    ax[index1].plot(
        ...
        fig.savefig(pwd + '/dosage_comparison_' + vox_size_list[fig_index] + '_suxer.pdf')
...
def isodose_plot():
    ...
    file_list = [
        target_dir +
        '/mlwa_0shield_{0}_sim_sux.phantom.3ddose',
        target_dir +
        '/mlwa_90shield_{0}_sim_sux.phantom.3ddose',
        target_dir +
        '/mlwa_180shield_{0}_sim_sux.phantom.3ddose',
    ...
    for fig_index in xrange(len(vox_size_list)):
        ...
        for file_index, file_name in enumerate(file_list):
            ...
            if file_index == 0:
                ...
                full_data = DoseFile(
                    file_name.format(vox_size_list[fig_index]))
        ...
        fig.savefig(
            pwd + '/xy_isodose_profile_' + vox_size_list[fig_index] + '_suxer.pdf'
            )
        ...
        fig2.savefig(
            pwd + '/xz_isodose_profile_' + vox_size_list[fig_index] + '_suxer.pdf'
            )
